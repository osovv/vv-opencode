// FILE: src/plugins/system-context-injection/agent-contexts.ts
// VERSION: 1.0.0
// START_MODULE_CONTRACT
//   PURPOSE: Own the shared system-context blocks and agent filters used by both the v1 chat.message hook and the v2 session context hook.
//   SCOPE: Pure constants and filters only: universal and explore system contexts, known subagent sets, injection eligibility, and per-agent context selection.
//   DEPENDS: [src/lib/managed-agents.ts, src/lib/orchestration.ts]
...
//
// START_MODULE_MAP
//   createKnownSubagentSet - Builds the built-in, plugin-managed, and vvoc-managed subagent name set.
//   shouldInjectForAgent - Decides whether one agent receives system context injection.
//   getSystemContextsForAgent - Returns the system context blocks for one agent and orchestration policy.
// END_MODULE_MAP
//
// START_CHANGE_SUMMARY
//   LAST_CHANGE: [C-OPENCODE-V2-MIGRATION T-004 - Extracted the shared agent context blocks and filters for dual-runtime reuse.]
// END_CHANGE_SUMMARY

import { MANAGED_SUBAGENT_NAMES } from "../../lib/managed-agents.js";
import type { ResolvedOrchestrationPolicy } from "../../lib/orchestration.js";
...
export function shouldInjectForAgent(
...
export function getSystemContextsForAgent(
  agentName: string | undefined,
