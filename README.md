...
Source-independent execution is built on one common contract. At this snapshot that contract, the execution registry, and the advance-authority domain are implemented and tested as internal workflow modules; the exposed registration path remains the native `work_checkpoint register` approved package, and exposing the generic registration, completion, and authority operations through the workflow tools is still in progress. The contract itself is designed so that requirements may come from a native package, a provided plan reference, or the current conversation, and the registry validates task and checkpoint identities, exact references, execution boundaries, and acyclic dependencies before anything is published. Required reviews are exactly those registered by the source, the user, or an explicit controller decision; a task with no assigned review completes honestly as controller-accepted rather than as independently reviewed, and a final checkpoint, when one was assigned, must still cover the current revision before completion. Explicit autonomous authority is recorded as a bounded advance reserve shared by the whole authorized execution, consumed one unit at a time only after ordinary allowances are exhausted; duplicate calls, replay, restart, and task replacement cannot refill it, and narrowing or revocation disables future credits without erasing history. An authoritative runtime that predates a stop can authorize one bounded continuation, while the separate post-stop `userMessageId` path keeps its own timing rules.
