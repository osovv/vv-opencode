...
- Public plugins: Guardian, Hashline Edit, Model Roles, System Context Injection,
  Workflow, and Secrets Redaction.
...
| Config resolution | `src/lib/config-layers.ts`, `src/lib/vvoc-paths.ts` | Env/project/global/default precedence and write targets. |
| Config document | `src/lib/vvoc-config.ts`, `schemas/vvoc/v3.json` | Strict canonical v3 parsing, rendering, and schema. |
| OpenCode mutation | `src/lib/opencode.ts` | Conservative registration and managed config writes. |
...
- Keep all root and plugin subpath exports in `package.json` importable.
