...
export type SummaryValidationResult = { ok: true; summary: string } | { ok: false; reason: string };
...
const AI_GENERATION_PATTERN =
  /\b(AI-generated|AI\s+generated|generated by (?:an?\s+)?(?:AI|artificial intelligence|LLM|language model|the model)|artificial intelligence generated|LLM[-\s]+generated|language model|written by AI)\b/i;
...
export function validateReleaseSummary(summary: string): SummaryValidationResult {
  ...
  if (trimmed.length > MAX_SUMMARY_LENGTH) {
    return {
      ok: false,
      reason: `Summary exceeds ${MAX_SUMMARY_LENGTH} characters (${trimmed.length}).`,
    };
  }
...
export function injectSummaryIntoChangelogEntry(changelogEntry: string, summary: string): string {
  ...
  return [...before, "", "### Summary", "", summary, "", ...body].join("\n");
}
...
export function validateLatestChangelogSummary(changelogText: string): SummaryValidationResult {
  ...
  const summaryText = (
    nextCommitOrHeader === -1 ? afterSummary : afterSummary.slice(0, nextCommitOrHeader)
  ).trim();
