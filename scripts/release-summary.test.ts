...
describe("buildReleaseSummaryAgentConfig", () => {
  test("returns valid JSON with restricted agent config", () => {
    ...
    expect(parsed.primary.name).toBe("release-summary");
    expect(parsed.primary.model).toBe("test/model");
    expect(parsed.primary.steps).toBe(1);
    expect(parsed.primary.permission["*"]).toBe("deny");
  });
});
...
describe("parseOpencodeRunJsonOutput", () => {
  test("accumulates text events into success", () => {
    const stdout = [
      makeTextEvent("This release "),
      makeTextEvent("adds new features."),
    ].join("\n");
    const result = parseOpencodeRunJsonOutput(validSummaryXml(stdout.replace(/\n/g, " ").replace(/^<summary>\s*/, "").replace(/\s*<\/summary>$/, "")));
    // Actually just test raw text accumulation with valid summary:
    const validStdout = validSummaryXml("This release adds new features.");
    const r = parseOpencodeRunJsonOutput(
      JSON.stringify({ type: "text", part: { type: "text", text: validStdout } }),
    );
